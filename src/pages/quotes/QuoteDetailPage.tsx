...
const QuoteDetailPage: React.FC = () => {
  ...
  const { user } = useAuth();
  const { getServiceIdByIdentifier } = useServices();
  ...
  const fetchQuote = async () => {
    try {
      ...
      if (!id || !user?.id) {
        throw new Error('Invalid quote ID or user not authenticated');
      }
  ...
  const handleAcceptQuote = async () => {
    try {
      ...
      const orderItem = {
        ...
        price: quote.admin_price || quote.facility_suggestion_price || quote.suggested_price,
        quantity: 1,
        is_custom_quote: false
      };

      // For custom quotes, we don't need a service ID
  ...
  const enhancedQuote = {
    ...
    estimated_days: quote.suggested_time || 3
  };
    ...
    <div className="min-h-screen pt-24 pb-12 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-gray-50 to-white">
      <div className="max-w-2xl mx-auto">
        <QuoteDetailView
          ...
          onBack={() => navigate('/account/quotes')}
        />
      </div>
    </div>
  );
