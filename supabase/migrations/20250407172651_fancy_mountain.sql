/*
  # Add order_id to custom_price_quotes table

  1. Changes
    - Add order_id column to custom_price_quotes table
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'custom_price_quotes' AND column_name = 'order_id'
  ) THEN
    ALTER TABLE custom_price_quotes ADD COLUMN order_id uuid REFERENCES orders(id) ON DELETE CASCADE;
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_custom_price_quotes_order_id ON custom_price_quotes(order_id);
...
-- Add comments
COMMENT ON COLUMN custom_price_quotes.order_id IS 'Reference to the order this quote belongs to';
